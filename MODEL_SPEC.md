...
Four baselines, implemented in `src/models/baselines.py`:
...
| `empirical` | historical **median** h-day log return | the next h days look like past h-day periods |

`empirical` was added after the first forecast run showed every horizon at or
beyond 30d predicting a median of exactly today's price. That is what
`no_change` means, but it exposed a gap: the other three all predict a median
derived from zero or from a *mean*, and none of them predicts the quantity that
actually minimises absolute error -- the median. BTC's long-horizon return
distribution is strongly right-skewed, so its mean and median are far apart, and
a baseline family that cannot express "the typical year was up 20%" was missing
an obvious hypothesis rather than having rejected it. It has now been tested
(section 6.2) and rejected on evidence.
...
MASE is each baseline's mean absolute log-return error relative to
`no_change` on the same origins; above 1.0 means worse.

| horizon | `no_change` MAE | `drift` MASE | `rolling_return` MASE | `empirical` MASE |
| --- | --- | --- | --- | --- |
| 1d | 0.0234 | 1.001 | 1.008 | 1.001 |
| 7d | 0.0660 | 1.002 | 1.027 | 1.002 |
| 30d | 0.1611 | 1.011 | 1.226 | 1.016 |
| 90d | 0.3345 | 1.012 | 1.393 | 1.061 |
| 180d | 0.4687 | 1.066 | 1.889 | 1.155 |
| 365d | 0.7196 | 1.127 | 2.472 | 1.084 |

**`no_change` is the reference and nothing displaces it.** No baseline beats it
on MASE at any horizon. `empirical` edges it on `pinball_mean` at 365d
(0.2515 vs 0.2583, 2.6%) but that sits on 4.3 independent windows at a
low-power horizon, which section 4.3 of VALIDATION_SPEC.md forbids acting on.

Three findings that constrain Phase 4:

1. **`no_change` is the bar, and it is not a weak one.** Nothing beats it
   anywhere except by noise. A tree model that does not beat it on
...
## 7.2 Forecast composition (blending)

Section 6.5 leaves a problem the daily forecast has to solve: the model is better
at h=1, the baseline is better from h=30, and something has to be served for
every horizon in between.

Serving the model everywhere would mean knowingly shipping a worse forecast at
most horizons because it came out of a model. A hard switch at one horizon would
put a visible step in the curve, and a reader cannot tell a modelling artefact
from a market view.

So each horizon is a weighted average of the two **quantile functions**
(Vincentization):
...
with `w` falling linearly from 1 to 0 across `forecast.blend`:

| horizon | model weight |
| --- | --- |
| 1d | 1.00 |
| 7d | 0.79 |
| 15d | 0.52 |
...
A convex combination of monotone functions is monotone, so blending cannot
introduce quantile crossing. Each stored forecast point records its `source`
(`model` / `blend` / `baseline`), so any number can be traced to what produced it.

**The weights are frozen**, set from inner validation and not re-derived per
forecast. A weight tuned on recent performance would be a model selected on the
data it is then scored against.

Implementation: `src/forecast/blending.py`, applied by `src/forecast/generate.py`.
...
Implementation: `src/forecast/interpolate.py`.

- **PCHIP, never a natural cubic spline.** A cubic spline through a sparse
  horizon grid overshoots between knots and invents local maxima the model never
  predicted; on a price chart that reads as a forecast of a rally nothing
  produced.
- **Interpolated in log-return space, then exponentiated**, which keeps every
  displayed price positive by construction and makes the curve's shape
  independent of the price level.
- **Anchored at `(h=0, return=0)`**, so the curve starts exactly at the last
  closed price. Without the anchor the chart shows a gap or a jump at the NOW
  marker.
- **Re-sorted per displayed date** after interpolation, which is how the
  no-value-outside-its-band requirement above is actually guaranteed rather than
  assumed from the knots.
- **Never persisted and never scored.** `forecast_points` holds only the grid
  horizons the model actually produced.
